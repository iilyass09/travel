...
@section('content')
  <div class="mb-6 flex justify-end">
    <a href="{{ route('admin.shopping.create') }}" class="px-6 py-3 bg-blue-600 text-white font-semibold rounded-xl hover:bg-blue-700 transition-colors">+ Tambah Shopping</a>
  </div>
  ...
  <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
      ...
      <div class="flex gap-2">
        <a href="{{ route('admin.shopping.edit', $item) }}" class="px-3 py-1.5 text-xs bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200">Edit</a>
        <form method="POST" action="{{ route('admin.shopping.destroy', $item) }}" onsubmit="return confirm('Hapus spot shopping ini?')">
          @csrf @method('DELETE')
    ...
    @empty
    <div class="col-span-full bg-white rounded-2xl p-8 text-center text-gray-400 border border-gray-100">Belum ada spot shopping.</div>
    @endforelse
  </div>
@endsection
