...
@section('body_class', 'detail-page font-body bg-white text-gray-900 overflow-x-hidden')
...
@section('content')
  ...
  <!-- Sub-nav sticky -->
  <nav class="detail-subnav bg-white/90 backdrop-blur-xl border-b border-gray-100 shadow-sm" aria-label="Navigasi paket">
  ...
  <!-- Itinerary -->
  <section id="itinerary" class="detail-anchor py-20 bg-gradient-to-b from-gray-50 to-white">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="text-center mb-14" data-aos="fade-up">
        <div class="inline-flex items-center gap-2 px-4 py-2 bg-green-100 rounded-full mb-4">
          ...
          <span class="text-green-700 font-semibold text-sm">Rencana Perjalanan</span>
        </div>
        <h2 class="font-heading text-3xl sm:text-4xl font-extrabold text-gray-900 mb-3">Itinerary <span class="text-transparent bg-clip-text bg-gradient-to-r from-emerald-500 to-green-600">{{ $package->name }}</span></h2>
        <p class="text-gray-600 text-lg max-w-2xl mx-auto">Rute perjalanan yang fleksibel dan bisa disesuaikan dengan keinginan Anda</p>
      </div>
      <div class="timeline">
        ...
        <div class="timeline-item" data-aos="fade-up" data-aos-delay="{{ $i * 100 }}">
          ...
          <div class="timeline-card">
            <div class="bg-white rounded-2xl p-5 shadow-lg shadow-gray-100 border border-gray-100">
              <div class="flex items-center gap-2 mb-1"><span class="px-3 py-1 bg-blue-100 text-blue-700 text-xs font-bold rounded-full">{{ $item->time }}</span><span class="text-gray-400 text-xs font-medium">{{ $item->label }}</span></div>
              <h3 class="font-heading font-bold text-lg text-gray-900 mb-1">{{ $item->title }}</h3>
  ...
  <!-- Fasilitas -->
  <section id="fasilitas" class="detail-anchor py-20 bg-white">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="text-center mb-14" data-aos="fade-up">
        <div class="inline-flex items-center gap-2 px-4 py-2 bg-blue-100 rounded-full mb-4">
          ...
          <span class="text-blue-700 font-semibold text-sm">Yang Termasuk</span>
        </div>
        <h2 class="font-heading text-3xl sm:text-4xl font-extrabold text-gray-900 mb-3">Fasilitas <span class="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-blue-800">Paket</span></h2>
        <p class="text-gray-600 text-lg max-w-2xl mx-auto">Transparan tanpa biaya tersembunyi</p>
      </div>
      <div class="grid md:grid-cols-2 gap-6" data-aos="fade-up">
        <div class="bg-emerald-50/60 border border-emerald-100 rounded-3xl p-6">
          <h3 class="font-heading font-bold text-lg text-emerald-700 mb-4 flex items-center gap-2"><svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>Sudah Termasuk</h3>
        ...
        <div class="bg-red-50/60 border border-red-100 rounded-3xl p-6">
          ...
          <p class="text-xs text-gray-500 mt-4 italic">Tiket masuk bisa diakomodasi dengan <a href="#pesan" class="text-blue-600 underline">Paket Plus</a> sesuai tujuan wisata Anda.</p>
        </div>
      </div>
    </div>
  </section>
  ...
  <!-- Harga -->
  <section id="harga" class="detail-anchor py-20 bg-gradient-to-b from-white to-gray-50">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="text-center mb-14" data-aos="fade-up">
        <div class="inline-flex items-center gap-2 px-4 py-2 bg-orange-100 rounded-full mb-4"><span class="text-orange-700 font-semibold text-sm">Transparan &amp; Hemat</span></div>
        <h2 class="font-heading text-3xl sm:text-4xl font-extrabold text-gray-900 mb-3">Pilihan <span class="text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-orange-600">Harga</span></h2>
        <p class="text-gray-600 text-lg max-w-2xl mx-auto">Harga per kendaraan (trip), bukan per orang. Bebas mobil, driver, dan BBM.</p>
      </div>
      <div class="grid md:grid-cols-3 gap-6" data-aos="fade-up">
        @foreach ($package->pricingTiers as $tier)
        <div class="relative bg-white rounded-3xl border-2 border-gray-200 p-6 shadow-lg flex flex-col">
          @if ($tier->badge)
          ...
          @endif
          <h3 class="font-heading font-bold text-xl text-gray-900 mb-1">{{ $tier->name }}</h3>
          <p class="text-gray-500 text-sm mb-4">{{ $tier->subtitle }}</p>
          <div class="font-heading text-3xl font-extrabold text-blue-700 mb-1">{{ $tier->price }}</div>
      ...
      <p class="text-center text-sm text-gray-500 mt-8" data-aos="fade-up"><span class="font-semibold text-blue-700">Catatan:</span> Harga dapat berubah sewaktu-waktu. Konfirmasi harga terbaik di WhatsApp kami.</p>
    </div>
  </section>

  <!-- Galeri -->
  <section id="galeri" class="detail-anchor py-20 bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="text-center mb-14" data-aos="fade-up">
        <div class="inline-flex items-center gap-2 px-4 py-2 bg-purple-100 rounded-full mb-4"><span class="text-purple-700 font-semibold text-sm">Galeri</span></div>
        <h2 class="font-heading text-3xl sm:text-4xl font-extrabold text-gray-900 mb-3">Serunya <span class="text-transparent bg-clip-text bg-gradient-to-r from-purple-500 to-indigo-600">{{ $package->name }}</span></h2>
      </div>
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        ...
        <div class="gallery-card relative rounded-2xl aspect-square" data-aos="zoom-in" data-aos-delay="{{ ($i % 4) * 100 }}">
          ...
          @endif
          @if ($photo->caption)
          <div class="absolute bottom-0 inset-x-0 p-4 @if(!$photo->image) bg-gradient-to-t from-black/40 to-transparent @endif"><p class="text-white font-heading font-bold text-sm md:text-base">{{ $photo->caption }}</p></div>
          @endif
        </div>
        @endforeach
      </div>
  ...
  <!-- Testimoni -->
  <section id="testimoni" class="detail-anchor py-20 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="text-center mb-14" data-aos="fade-up">
        <div class="inline-flex items-center gap-2 px-4 py-2 bg-yellow-100 rounded-full mb-4"><span class="text-yellow-700 font-semibold text-sm">Kata Mereka</span></div>
        <h2 class="font-heading text-3xl sm:text-4xl font-extrabold text-gray-900 mb-3">Testimoni <span class="text-transparent bg-clip-text bg-gradient-to-r from-yellow-500 to-orange-600">Wisatawan</span></h2>
      </div>
      <div class="grid md:grid-cols-3 gap-6">
        @foreach ($package->testimonials as $i => $t)
        <div class="bg-white rounded-3xl p-6 shadow-lg shadow-gray-100 border border-gray-100" data-aos="fade-up" data-aos-delay="{{ $i * 100 }}">
          ...
          <div class="flex items-center gap-3"><div class="w-11 h-11 rounded-full bg-gradient-to-br {{ $t->avatar_gradient }} flex items-center justify-center text-white font-bold font-heading">{{ $t->initials() }}</div><div><p class="font-heading font-bold text-sm text-gray-900">{{ $t->name }}</p><p class="text-gray-400 text-xs">{{ $t->role }}</p></div></div>
        </div>
        @endforeach
      </div>
    </div>
  ...
  <!-- FAQ -->
  <section id="faq" class="detail-anchor py-20 bg-white">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="text-center mb-14" data-aos="fade-up">
        <div class="inline-flex items-center gap-2 px-4 py-2 bg-blue-100 rounded-full mb-4"><span class="text-blue-700 font-semibold text-sm">Pertanyaan Umum</span></div>
        <h2 class="font-heading text-3xl sm:text-4xl font-extrabold text-gray-900 mb-3">FAQ <span class="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-blue-800">{{ $package->name }}</span></h2>
      </div>
  ...
  <section class="py-20 bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="text-center mb-12" data-aos="fade-up">
        <h2 class="font-heading text-3xl font-extrabold text-gray-900 mb-2">Paket Wisata <span class="text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-orange-600">Lainnya</span></h2>
        <p class="text-gray-600">Lanjutkan petualangan Anda ke destinasi favorit lainnya</p>
      </div>
  ...
  <div id="sticky-book" class="md:hidden fixed bottom-0 left-0 right-0 z-[85] bg-white/95 backdrop-blur-xl border-t border-gray-100 px-4 py-3 flex items-center justify-between gap-4">
    <div class="min-w-0"><p class="text-xs text-gray-500">Mulai dari</p><p class="font-heading font-bold text-blue-700">{{ $package->price ?? 'Custom' }} @if($package->price_compare)<span class="text-gray-400 line-through text-sm font-normal">{{ $package->price_compare }}</span>@endif</p></div>
